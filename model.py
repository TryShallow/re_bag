...
class Model(nn.Module):
    ...
    def forward(self, x):
        query_length = x['query_length_mb']#.to(self.device)
        nodes_elmo = x['nodes_elmo_mb']#.to(self.device)
        query_elmo = x['query_elmo_mb']#.to(self.device)
        nodes_length = x['nodes_length_mb']#.type(torch.int32).to(self.device)
        adj = x['adj_mb']#.type(torch.float32).to(self.device)
        bmask = x['bmask_mb']#.type(torch.float32).to(self.device)
